  ...
  <div class="comm-manage administration">
    ...
    <div class="right-view">
      <div class="top-search">
        <div class="tabBtn">
          <el-button-group>
            <el-button :type="typeState == '0' ? 'primary' : 'default'" @click="changeState('0')">正常门店</el-button>
            <el-button :type="typeState == '1' ? 'primary' : 'default'"  @click="changeState('1')" >已关闭门店</el-button>
          </el-button-group>
        </div>
        <search @searchValue="searchValue" :load="loadingSerch"></search>
        <div class="busi-btns">
          <el-button class="btn newcom-btn" @click="addStore" data-code="10010002" ref="clean">新门店</el-button>
        </div>
      </div>
        ...
        <div class="detail-item">
          <el-table
            ...
            <el-table-column
              ...
              label="经营品牌">
              <template slot-scope="scope" >
                <div v-for="(item, index) in scope.row.brandName" :key="index" :item="item" v-if="index <= 3" class="brand-list">
                  <span>{{item}}</span><span v-if="index+1 < scope.row.brandName.length"> , </span>
                </div>
                <div  v-if="scope.row.brandName !== null && scope.row.brandName.length > 3" class="brand-list">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div class="comm-opera">
                  ...
                  <el-button round v-if="scope.row.state === '1'" type="success" @click="OpenStore('0',scope.row.storeId)"  data-code="10010004" :loading="loading">{{loading? '开店中' : '开店'}}</el-button>
                  <el-button round v-else type="danger"  @click="StopStore('1',scope.row.storeId)"  data-code="10010005" :loading="loading">{{loading? '关店中' : '关店'}}</el-button>
                </div>
            </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    changeState (storeState3) {
      console.log(storeState3,'---------------',this.typeState);
      if (storeState3 != this.typeState) {
        this.typeState = storeState3;
        this.getListData();
      } else {
        console.log('没有变化...');
      }
    ...
    // 分页查询和模糊搜索
    getListData () {
      this.storevo.state = this.typeState
