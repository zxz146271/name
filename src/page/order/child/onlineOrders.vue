<template>
  <div class="order-manage">
    <div class="right-view">
      <div class="top-search">
        <div class="serch-left">
          <el-select v-model="type" placeholder="请选择" @change="changeOrderType(type)">
            <el-option
              v-for="item in condition"
              :key="item.type"
              :label="item.title"
              :value="item.type"
            ></el-option>
          </el-select>
          <search @searchValue="searchValue" :load="loading" class="search"></search>
        </div>
        <div class="busi-btns">
            <el-button
                class="btn import-btn"
                @click="addOrder"
                data-code="6010001"
                v-if="orderType == 'ORDER'"
            >+新订单</el-button>
        </div>
      </div>
      <div class="operate">
        <div class="item">
          <div class="letf-op">
            <!-- <el-checkbox v-model="checkAllComm" v-show="page !== '6'">全选</el-checkbox> -->
            <el-checkbox>全选</el-checkbox>
          </div>
          <div class="right-op">
            <span class="sort" @click="toggle">价格
              <i v-bind:class="{'up': isA}"></i>
              <i v-bind:class="{'down': !isA}"></i>
            </span>
          </div>
        </div>
      </div>
      <div class="comm-list">
        <div class="content">
            <div class="title">
                <span>商品详情</span>
                <span>送货/提货日期</span>
                <span>收货信息</span>
                <span>总金额</span>
                <span>已交付金额</span>
                <span>未交付金额</span>
            </div>
            <onlineOrderItem v-for="(item, index) in onStoreOrderList" :key="index" :item="item" />
        </div>
        
      </div>
      <div class="pagin">
        <span class="total">共{{totalCount}}条数据</span>
        <el-pagination
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
          :current-page.sync="currentPage"
          layout="sizes, prev, pager, next"
          :page-sizes="[2, 5, 10, 20]"
          :total="totalCount"
        ></el-pagination>
      </div>
    </div>
  </div>
</template>
<script>
import { mapGetters } from "vuex";
import search from "../../../components/public/search";
import leftCondition from "../../../components/public/leftCondition";
// import steps from '../../../components/public/steps'
// import topOperate from '../../../components/order/orderTopOperate'
import orderManageItem from "../../../components/order/orderManageItem";
import onlineOrderItem from "../../../components/order/onlineOrderItem";
import { exportJsonExcel } from "../../../vendor/Export2Excel";
import { judgeButton, getStore } from "../../../tool/Utils";
export default {
  data() {
    return {
      loading: false,
      checkAllOrder: false,
      importComm: false,
      active: 0,
      optionTop: {
        type: "1"
      },
      order: {
        orderId: this.$route.query.orderId,
        state: "",
        rank: "defualdesc"
      },
      list: [
        {
          name: "韩版设计时尚风衣大",
          number: "MPM00112",
          salePrice: "￥999.00",
          stocknums: 3423,
          salesnums: 3423,
          sharenums: 3423
        }
      ],
      isA: false,
      num: true,
      isB: true,
      type: 0,
      condition: [
        {
          type: 0,
          title: "全部订单",
          isActive: true
          // num: 15
        },
        {
          type: 1,
          title: "待付款",
          isActive: false
          // num: 10
        },
        {
          type: 2,
          title: "待发货",
          isActive: false
          // num: 20
        },
        // {
        //   type: 4,
        //   title: '部分发货',
        //   isActive: false,
        //   num: 15
        // },
        {
          type: 3,
          title: "已发货",
          isActive: false
          // num: 0
        },
        {
          type: 4,
          title: "已完成",
          isActive: false
          // num: 8
        },
        // {
        //   type: 5,
        //   title: '退货中',
        //   isActive: false
        //   // num: 0
        // },
        {
          type: 6,
          title: "已取消",
          isActive: false
          // num: 6
        },
        {
          type: 7,
          title: "部分付款",
          isActive: false
          // num: 10
        },
        {
          type: 8,
          title: "部分发货",
          isActive: false
          // num: 10
        }
      ],
      totalCount: 0,
      currentPage: 1,
      pagesize: 10,
      // 列表声明一个空数组
      stStoreOrderList: [],
      onStoreOrderList: [
        {
          goodsName: "现代时尚简约沙发黄色款XXXXXXXXXX", //商品名称
          submitTime: "2019-01-02  16:46:23", //提交时间
          deliveryTime: "2019-01-02  16:46:23", //送货时间
          pickUpTime: "2019-01-02  16:46:23", //提货时间
          consignee: "小花", //提货人
          moble: "13618255687", //提货人
          shippingAddress: "上海市浦东新区芦恒路附近三公里范围自己找", //提货地址
          totalAmount: 2888.0, //总金额
          postage: 0, //邮费
          paidAmount: 0, //已交付金额
          unPaidAmount: 2888.00, //未交付金额
          orderStatus: 0, //---0 未付款
          firstPictureUrl: "../../../../static/image/2019.jpg", //---
          defaultImg: "../../../../static/image/huodonghong_icon@2x.png", //---
        },
        {
           goodsName: "现代时尚简约沙发黄色款XXXXXXXXXX", //商品名称
          submitTime: "2019-01-02  16:46:23", //提交时间
          deliveryTime: "2019-01-02  16:46:23", //送货时间
          pickUpTime: "2019-01-02  16:46:23", //提货时间
          consignee: "小花", //提货人
          moble: "13618255687", //提货人
          shippingAddress: "上海市浦东新区芦恒路附近三公里范围自己找", //提货地址
          totalAmount: 2888.0, //总金额
          postage: 0, //邮费
          paidAmount: 0, //已交付金额
          unPaidAmount: 2888.00, //未交付金额
          orderStatus: 0, //---0 未付款
          firstPictureUrl: "../../../../static/image/2019.jpg", //---
          defaultImg: "../../../../static/image/huodonghong_icon@2x.png", //--- 
        }
      ],
      tableData2: [],
      orderType: ""
    };
  },
  // 数据初始化方法
  mounted() {
    this.getStStoreOrderList();
    this.getButton();
  },
  watch: {
    checkAllOrder() {
      this.checkAll(this.checkAllOrder);
    },
    itemsChoose: {
      handler(newValue, oldValue) {
        if (newValue.length === 0 && this.stStoreOrderList.length > 0) {
          this.checkAllOrder = true;
        } else if (newValue.length === this.stStoreOrderList.length) {
          this.checkAllOrder = false;
        } else if (
          newValue.length !== 0 &&
          newValue.length < this.stStoreOrderList.length
        ) {
          this.checkAllOrder = false;
        }
      },
      deep: true
    }
  },
  computed: {
    ...mapGetters(["powerButton"]),
    itemsChoose() {
      let isAllSele = [];
      this.stStoreOrderList.forEach(item => {
        if (item.isChooe === false) {
          isAllSele.push(false);
        }
      });
      return isAllSele;
    }
  },
  created() {
    this.getOrderStyle();
  },
  methods: {
    getOrderStyle() {
      let storeId = getStore("storeIdWeb");
      let params = {
        storeId: storeId
      };
      this.$api.getOrderStyle(params).then(res => {
        if (res.code === 0) {
          this.orderType = res.data;
        }
      });
    },
    // 模糊查询
    searchValue(value) {
      this.order.orderId = value;
      this.currentPage = 1;
      this.loading = true;
      this.getStStoreOrderList();
    },
    changeOrderType(index) {
      if (index === 0) {
        this.order.state = "";
      } else {
        this.order.state = index + "";
      }
      this.getStStoreOrderList();
    },
    getButton() {
      let pageButtons = document.getElementsByTagName("button");
      judgeButton(pageButtons, this.powerButton);
    },
    refresh1() {
      this.getStStoreOrderList();
    },
    toggle() {
      this.isA = !this.isA;
      if (this.num) {
        this.num = false;
        this.order.rank = "defualdesc";
      } else {
        this.num = true;
        this.order.rank = "defualasc";
      }
      this.getStStoreOrderList();
    },
    toggle2() {
      this.isB = !this.isB;
      if (this.num) {
        this.num = false;
        this.order.rank = "desc";
      } else {
        this.num = true;
        this.order.rank = "asc";
      }
      this.getStStoreOrderList();
    },
    // refresh () {
    //   this.getStStoreOrderList()
    // },
    // 获取订单信息列表
    getStStoreOrderList(value) {
      let params = {
        page: this.currentPage,
        limit: this.pagesize,
        order: this.order
      };
      this.$api.getStStoreOrderList(params).then(res => {
        if (res.code === 0) {
          this.loading = false;
          this.totalCount = res.count;
          res.data.forEach(function(order) {
            var str = order.createTime.substring(0, 10);
            if (order.reservationDate) {
              var str1 = order.reservationDate.substring(0, 10);
            }
            order.createTime = str;
            order.reservationDate = str1;
            if (order.provinceAdcode) {
              JSON.parse(order.provinceAdcode).forEach(el => {
                if (order.provinceAdcode.indexOf("[") !== -1) {
                  order.provinceAdcode = "";
                  order.provinceAdcode = el;
                } else {
                  order.provinceAdcode += el;
                }
              });
            }
            order.isChooe = false;
            order.Address = order.provinceAdcode + order.detailAddress;
            if (order.state === "1") {
              order.type = "待付款";
            } else if (order.state === "2") {
              order.type = "待发货";
            } else if (order.state === "3") {
              order.type = "已发货";
            } else if (order.state === "4") {
              order.type = "已完成";
            } else if (order.state === "5") {
              order.type = "退货中";
            } else {
              order.type = "已关闭";
            }
          });
          this.stStoreOrderList = res.data;
        } else {
          this.loading = false;
          this.$message.error(res.msg);
        }
      });
    },
    // 全选
    checkAll(param) {
      if (param) {
        this.stStoreOrderList.forEach(el => {
          el.isChooe = true;
        });
      } else if (this.itemsChoose.length === 0 && !param) {
        this.stStoreOrderList.forEach(el => {
          el.isChooe = false;
        });
      }
    },
    addOrder() {
      this.$router.push({ path: "/myStore/order/orderAdd" });
    },
    handleClose() {
      this.importComm = false;
    },
    handleSizeChange: function(size) {
      this.pagesize = size;
      this.getStStoreOrderList();
    },
    handleCurrentChange: function(currentPage) {
      this.currentPage = currentPage;
      this.getStStoreOrderList();
    },
    current_change: function(currentPage) {
      this.currentPage = currentPage;
    },
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => v[j]));
    },
    // 表格导出
    exportExcel() {
      this.$emit("exportExcel");
      const tHeader = [
        "订单号",
        "开单日期",
        "收货人",
        "收货人电话",
        "收货人地址",
        "收货日期",
        "总金额",
        "已交付金额",
        "未交付金额",
        "订单状态",
        "开单门店",
        "开单人"
      ];
      const filterVal = [
        "orderNum",
        "createTime",
        "consignee",
        "consigneeMobile",
        "Address",
        "reservationDate",
        "totalMoney",
        "accountPaid",
        "nonPayment",
        "type",
        "storeName",
        "staffName"
      ];
      var flag = false;
      this.stStoreOrderList.forEach(el => {
        if (el.isChooe) {
          flag = true;
          this.tableData2.push(el);
        }
      });
      if (!flag) {
        this.$message.error("请至少选择一条订单信息进行导出");
      } else {
        const data = this.formatJson(filterVal, this.tableData2);
        exportJsonExcel(tHeader, data, "订单管理列表");
      }
    }
  },
  components: {
    search,
    orderManageItem,
    leftCondition,
    onlineOrderItem
  }
};
</script>
<style lang="scss" scoped>
@import "../../../assets/style/mixins/var";
@import "../../../assets/style/components/icon";
.order-manage {
  margin: 0 auto;
  display: flex;
  padding-top: 126px;
  .left-view {
    border: 1px solid $border-color;
    flex: 0 0 156px;
    padding: 21px 0 10px 36px;
    height: 370px;
    border-radius: 4px;
  }
  .right-view {
    flex: 1;
    // margin-left: 10px;
    .top-search {
      display: flex;
      justify-content: space-between;
      .serch-left {
        display: flex;
        align-items: center;
        .search {
          margin-left: 15px;
        }
      }
      .busi-btns {
        .btn {
          padding: 9px 20px;
          &.newcom-btn {
            margin-left: 0px;
            background: $blue-color;
            border-color: $blue-color;
            color: #fff;
            &:focus,
            &:hover {
              background: lighten($blue-color, 6%);
              border-color: lighten($blue-color, 6%);
            }
          }
          &.import-btn {
            background: #fd4343;
            border-color: #fd4343;
            color: #fff;
            &:focus,
            &:hover {
              background: lighten(#fd4343, 6%);
              border-color: lighten(#fd4343, 6%);
            }
          }
        }
      }
    }
  }
  .import-dialog {
    .left-title {
      position: absolute;
      left: 0;
      color: #fff;
      .top {
        position: relative;
        width: 82px;
        height: 28px;
        display: inline-block;
        background-color: $gray-color;
        line-height: 28px;
        text-align: center;
        color: $text-main-color;
        font-size: $xs-size;
        &::after {
          content: "";
          @include triangle-right(14px, 12px, $gray-color);
          position: absolute;
          right: -24px;
        }
        &::before {
          content: "";
          position: absolute;
          left: 0px;
          width: 4px;
          background-color: $main-color;
          height: 100%;
        }
      }
    }
  }
  .operate {
    border: 1px solid $border-color;
    padding: 6px 10px;
    margin: 20px 0 10px 0;
    border-radius: 4px;
    font-size: $xs-size;
    .item {
      display: flex;
      justify-content: space-between;
      align-items: center;
      .letf-op {
        display: flex;
        .el-checkbox {
          &:nth-child(2) {
            margin-left: 25px;
          }
        }
        .comm-up {
          position: relative;
          margin-left: 25px;
          display: flex;
          align-items: center;
          span {
            margin-right: 18px;
            cursor: pointer;
            i {
              font-size: 14px;
              position: relative;
              top: 1px;
              margin-right: 2px;
            }
            &:hover {
              color: $main-color;
            }
          }
        }
      }
      .right-op {
        button {
          padding: 5px 10px;
          &.history-btn {
            border: 0;
          }
          &.import-btn {
            border-color: $blue-color;
            background-color: $blue-color;
            color: #fff;
            &:hover {
              border-color: lighten($blue-color, 6%);
              background-color: lighten($blue-color, 6%);
            }
          }
        }
      }
      .middle-op {
        span {
          margin: 0 10px;
        }
      }
      .sort {
        cursor: pointer;
        i.up {
          position: relative;
          top: 2px;
          margin-left: 4px;
          @include icon(
            9px,
            14px,
            "../../../assets/image/icon/icon-price-up.png"
          );
        }
        i.down {
          position: relative;
          top: 2px;
          margin-left: 4px;
          @include icon(
            9px,
            14px,
            "../../../assets/image/icon/icon-price-down.png"
          );
        }
      }
    }
  }
  .title{
    display: flex;
    font-size: $xs-size;
    color:$text-main-color;
    background: $child-nav-bg;
    padding: 10px 0;
    border-bottom:1px solid $border-color;
    position: relative;
    span{
      font-weight: 700;
      line-height: 23px;
    //   text-align: center;
      &:nth-child(1){
        width: 29%;
        padding-left: 44px;
      }
      &:nth-child(2){
        // padding-left: 24px;
        width: 13.3%;
      }
      &:nth-child(3){
        padding-left: 24px;
        width: 15.6%;
      }
      &:nth-child(4){
        padding-left: 24px;
        width: 12.5%;
      }
      &:nth-child(5){
        padding-left: 24px;
        width: 12.5%;
      }
      &:nth-child(6){
        padding-left: 24px;
        width: 12.5%;
      }
    }
  }
}
</style>
