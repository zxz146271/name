      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="ruleForm">
        ...
        <el-col :span="24" v-if="orderJSON.title === '变更价格'">
          <el-form-item label= "变更价格" prop="payment">
             ...
             <font style="color: gray; font-size: 100%; line-height: 16px">*变更价格,不可小于已收金额</font>
          </el-form-item>
        </el-col>
        <el-col :span="24" v-if="orderJSON.title === '取消订单'  && orderJSON.orderState != '1' ">
          <el-form-item label= "退款金额" prop="payment" v-if="order.accountPaid !== 0">
            <div class="all-item">
              <el-input v-model="ruleForm.payment"></el-input>&nbsp;&nbsp;
              <span class="spanStyle" @click="clickPayMent" v-show="ruleForm.payment != order.accountPaid">全部</span>
            </div>
            <font style="color: gray; font-size: 100%; line-height: 16px">*退款金额不可大于已收金额,全部退款订单将被取消,部分退款订单完成</font>
          </el-form-item>
        </el-col>
        <el-col :span="24" v-if="(orderJSON.title === '取消订单' && orderJSON.orderState != '1') || orderJSON.title === '变更总价'">
      ...
      this.$refs[ruleForm].validate((valid) => {
        ...
        if (valid) {
          ...
          if (this.orderJSON.title === '变更价格') {
            ...
            this.$api.editeasyOrderTotalMoneySave(params).then(res => {
