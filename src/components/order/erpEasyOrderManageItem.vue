    ...
    <div class="order-item type0" v-if="'1' === item.state">
      ...
      <div class="bottom-op">
        ...
        <div style="display:none;">
          ...
          <el-button class="butn" @click="orderEdit(item.orderId)" data-code="6030006"> <i class="el-icon-edit-outline"></i> 编辑</el-button>
    ...
    <div class="order-item type2" v-if="'3' === item.state">
      ...
      <div class="bottom-op">
        ...
        <div style="display:none;">
          ...
          <el-button class="butn btn-type2" @click="confirm(item)" data-code="6030004" > <i class="el-icon-edit"></i>确认收货</el-button>
          <el-button class="butn" @click="colseAll(item.orderId)"  data-code="6030009" > <i class="iconfont el-icon-circle-close-outline"></i>关闭订单</el-button>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    confirm (value) {
      if (value.nonPayment !== 0) {
        this.$confirm('该订单还未全额付款, 是否继续收货?', '提示', {
          ...
          this.$api.confirm(params).then(res => {
            if (res.code === 0) {
              this.$message({
                message: '该订单已完成收货',
                type: 'success'
        ...
        this.$confirm('确认是否要进行确认收货操作, 是否继续收货?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
    ...
    // 关闭订单
    colse (value) {
      this.$confirm('该订单已在进行中, 是否继续关闭?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
