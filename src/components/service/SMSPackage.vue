    ...
    <div v-for="(item, index) in valueAddedServiceList" :key="index" :item="item" class="service-item">
      <div >
        <div class="  public" >
          <p>{{item.mealName}}</p>
          <ul class="ul-list">
            <li >{{item.mealRemarks}}</li>
          </ul>
          <div class="btn">
            <!-- <el-button class="gray-color" v-if="item.authorizationState==='1'" @click="childclick(item)">￥{{item.servicePrice}}元/{{changeData(item.billingMethod)}} 续费</el-button> -->
            <el-button class="blue-color"  @click="childclick(item)">￥{{item.mealAmount/100}}元/{{item.mealNum}}条 购买</el-button>
          </div>
        </div>
        <!-- <div class="bg-img">
...
export default{
  ...
  methods: {
    ...
    getSMSBuyList () {
      this.$api.getSMSBuyList().then(res => {
        this.valueAddedServiceList = res.data
      })
    },
    childclick (serviceItem) {
      // this.$emit('payment', serviceItem)
