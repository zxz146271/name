<template>
  <div class="com-item">
    ...
    <div class="btns">
        <el-button @click="addProductsClose()">取消</el-button>
        <el-button type="primary" @click="addProductOk()">确定</el-button>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    addProductOk() {
        this.goods.forEach(item => {
            if(item.choose == true){
                this.checkedDood.push(item)
            }
        })
        this.$emit("addProductOk", this.checkedDood)
        this.checkedDood = []
    }
  }
}
