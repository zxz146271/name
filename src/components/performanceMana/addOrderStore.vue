              ...
              <el-col :span="24" v-show="ruleForm.type">
               ...
               <el-col :span="24" >
                <el-form-item label="老板的目标业绩" >
                  <el-input v-model="ruleForm.expectationOrder" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="24" class="upload-item label-star">
                <el-form-item  v-for="(item, index) in staffData" :key="index" :label="item.staffName" >
...
export default {
  ...
  methods: {
    ...
    changeWeek (value) {
      this.ruleForm.expectationOrder = this.weekData[value].expectationOrder
      this.ruleForm.week = this.weekData[value].week
    },
    changeMonth (value) {
      this.ruleForm.expectationOrder = this.monthData[value].expectationOrder
      this.ruleForm.month = this.monthData[value].month
    },
    changeType (value) {
      // this.ruleForm.expectationOrder = 0
      this.ruleForm.month = ''
      this.ruleForm.week = ''
      if (value === '1') {
        this.flag = false
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.flag5) {
            ...
            if (count < this.ruleForm.expectationOrder) {
              ...
              let params = {
                ...
                orderNum: this.memberNum
              }
              if (this.submint) {
                this.$api.addSaveOrderperformance(params).then(res => {
                  this.submint = false
                  if (res.code === 0) {
                    this.submint = true
                    this.loading = false
