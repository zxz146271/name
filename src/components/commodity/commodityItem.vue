<template>
  <div class="com-item">
    <div class="left">
     ...
     <el-checkbox class="comm-check" @change="checkboxchange" :disabled="item.useState" v-model="item.choose"></el-checkbox>
    <div class="com-img">
        <img :src="item.firstPictureUrl" :onerror="defaultImg" alt="" >
        <!-- <div class="no-img" v-else>暂无商品图片</div> -->
      </div>
    </div>
    <div class="right">
      <div class="topic">
        <!-- <div class="ercode" v-if="$route.path==='/store/commManage'">
     ...
     <!-- <div class="comm-label" v-if="$route.path==='/store/commManage'">
        <span class="label">滞销
        ...
        <span class="label">主推货
          <i class="el-icon-delete"></i>
        <span class="label"  v-for="(name, index) in item.labelList" :key="index" v-if="name !== '新品' || name !== '滞销' || name !== '热销' || name !== '热门'">{{name}}
          <i class="el-icon-delete" @click="deleteLaber(name)"></i>
        </span>
        <span class="label"  v-for="(name, index) in item.labelList" :key="index" v-else>{{name}}
        </span>
         <template v-for="name in item.labelList"  :class="label">{{name}}</template>
        <div class="add-label">
          <span class="add-btn" @click="addLable" v-show ="!isAddLabel" data-code="5010007">+ 添加</span>
    ...
    <div class="comm-opera">
      ...
      <el-button round @click="setGoodsDrop(item.goodsId,item.state)" v-if="item.state!=='4' && $route.path !== '/marketing/storeMarket/marketDiscount' && $route.path !== '/marketing/storeMarket/marketRob' && item.state !== '6'" data-code="5010006">下架</el-button> -->
          <el-button round @click="setGoodsDrop(item.goodsId,item.state)" v-if="item.state==='4' && item.state !== '6'" data-code="5010004">上架</el-button>
      <el-button round @click="getActivityByGoodsId(item.goodsId,item.state)" v-if="item.state!=='4' && $route.path !== '/marketing/storeMarket/marketDiscount' && $route.path !== '/marketing/storeMarket/marketRob' && item.state !== '6'" data-code="5010004">下架</el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  data () {
    return {
      ...
      tempPrice: '',
      laberName: '',
      defaultImg: 'this.src="' + require('../../assets/image/image_placeholder.png') + '"'
    }
  },
  computed: {
